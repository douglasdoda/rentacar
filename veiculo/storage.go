...
//func (dono) nome Funcao (parametros) (retorno)
...
func (s *MySQLStorage) CreateVeiculo(nome, marca string, ano, modelo int) error {
	...
	//prepare o banco para receber os parametros
	stmt, err := s.dbConn.Prepare(insert)
	if err != nil {
		return err

	}
	//garante que será fechada a conexao
	defer stmt.Close()
	//executa parametros na query (banco)
	_, err = stmt.Exec(nome, marca, ano, modelo)
	if err != nil {
		return err
	}
	//se tudo ocorrer bem retornará NIL Pointer, ou seja, sem erro
	return nil
}
func (s *MySQLStorage) UpdateVeiculo(id int, veiculo *Veiculo) error {
	...
	defer stmt.Close()
	_, err = stmt.Exec(veiculo.Nome, veiculo.Nome, veiculo.Ano, veiculo.Modelo, id)
	if err != nil {
		return err
	}
	return nil
